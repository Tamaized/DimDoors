...
public class DDLoot {
	...
	private static final UUID[] SPECIAL_SKULL_OWNERS = new UUID[]{

			// Originals

			UUID.fromString("20961916-31df-4465-9f25-59b5d0d9d853"), // stevenrs11

			UUID.fromString("7cf8ccae-1d0f-425c-a7f2-c49ef18b2769"), // kamikazekiwi3

			UUID.fromString("74778202-78bf-401e-b890-e6088d48f163"), // fbt

			UUID.fromString("996d66c7-def8-4261-b7b3-e829dd338e53"), // Jaitsu

			UUID.fromString("b97e12ce-dbb1-4c0c-afc8-132b708a9b88"), // XCompWiz

			//UUID.fromString(""), // skyboy026 TODO unknown user

			UUID.fromString("31ba31b7-daa8-475f-853e-bad593f30808"), // Wylker

			// New

			UUID.fromString("16fea09e-314e-4955-88c2-6b552ecf314a"), // Tamaized

			UUID.fromString("eb21559e-bb22-46f2-897b-71eee2d5c09b") // TehNut
	...
	//	public static ChestGenHooks DungeonChestInfo = null; TODO
	private static final int CHEST_SIZE = 6;

	private DDLoot() {
	}

	// TODO, lets just comment all this out, we'll use a loot table later
	...
	private static void addModifiedEquipment(Item item, ArrayList<ItemStack> stacks, Random random) {
		...
		stacks.add(getModifiedItem(item, random, new Enchantment[]{Enchantments.BLAST_PROTECTION, Enchantments.FIRE_PROTECTION, Enchantments.PROTECTION, Enchantments.PROJECTILE_PROTECTION}));
	}
	...
	private static void addModifiedSword(Item item, ArrayList<ItemStack> stacks, Random random) {
		...
		stacks.add(getModifiedItem(item, random, new Enchantment[]{Enchantments.FIRE_ASPECT, Enchantments.KNOCKBACK, Enchantments.SHARPNESS}));
	}
	...
	private static void addModifiedTool(Item tool, ArrayList<ItemStack> stacks, Random random) {
		...
		stacks.add(getModifiedItem(tool, random, new Enchantment[]{Enchantments.EFFICIENCY, Enchantments.UNBREAKING}));
	}

	private static void addModifiedBow(ArrayList<ItemStack> stacks, Random random) {
		stacks.add(getModifiedItem(Items.BOW, random, new Enchantment[]{Enchantments.FLAME, Enchantments.POWER, Enchantments.PUNCH}));
	}
	...
	private static ItemStack applyRandomDamage(Item item, Random random) {
		int damage = (int) (item.getMaxDamage() * RandomBetween.getRandomDoubleBetween(random, MIN_ITEM_DAMAGE, MAX_ITEM_DAMAGE));
