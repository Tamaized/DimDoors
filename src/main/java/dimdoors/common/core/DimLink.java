...
public abstract class DimLink {
	...
	protected DimLink(DimensionPos point, int orientation, DDLock lock, DimLink parent) {
	...
	protected DimLink(DimensionPos point, int orientation, DDLock lock, LinkType linkType) {
	...
	public DimensionPos destination() {
	...
	public ChunkPos getChunkCoordinates() {
		return new ChunkPos(point.getX() >> 4, point.getZ() >> 4);
	}
