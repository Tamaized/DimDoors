...
public class DDStructureNetherBridgeStart extends StructureStart {
	...
	public DDStructureNetherBridgeStart(World world, Random random, int chunkX, int chunkZ) {
		...
		if (random.nextInt(MAX_GATEWAY_GENERATION_CHANCE) < DimDoorsConfig.fortressGatewayGenerationChance) {
			// Search for all the blaze spawners in a fortress
			spawnerRooms = new ArrayList<>();
	...
	@Override
	public void readStructureComponentsFromNBT(@Nonnull World world, NBTTagCompound fortressTag) {
	 ...
	 */
	@Override
	public void generateStructure(@Nonnull World world, @Nonnull Random random, @Nonnull StructureBoundingBox generationBounds) {
		if (hasGateway) {
