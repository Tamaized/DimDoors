...
public class LimboGenerator implements IChunkGenerator {
	...
	public Chunk generateChunk(int x, int z) {
		...
		LimboGenerator.rand.setSeed(x * 341873128712L + z * 132897987541L);
		ChunkPrimer primer = new ChunkPrimer();
		this.func_147424_a(x, z, primer);
		Chunk var4 = new Chunk(this.world, primer, x, z);
		var4.generateSkylightMap();

		if (!var4.isTerrainPopulated()) {
			var4.setTerrainPopulated(true);
			CustomLimboPopulator.registerChunkForPopulation(world.provider.getDimension(), new ChunkPos(x, z));
		}
		return var4;
	}
	...
	public void func_147424_a(int par1, int par2, ChunkPrimer primer) {
		byte var4 = 4;
		byte var5 = 16;
		byte var6 = 19;
		int var7 = var4 + 1;
		byte var8 = 17;
		int var9 = var4 + 1;
		this.biomesForGeneration = this.world.getBiomeProvider().getBiomesForGeneration(this.biomesForGeneration, par1 * 4 - 2, par2 * 4 - 2, var7 + 5, var9 + 5);
	...
	public List<Biome.SpawnListEntry> getPossibleCreatures(EnumCreatureType creatureType, BlockPos pos) {
		...
		return biomegenbase == Biomes.SWAMPLAND && creatureType == EnumCreatureType.MONSTER && this.scatteredFeatureGenerator.isPositionInStructure(world, pos) ? this.scatteredFeatureGenerator.getScatteredFeatureSpawnList() : biomegenbase.getSpawnableList(creatureType);
	}
