...
import dimdoors.common.dungeon.pack.DungeonPackConfigReader;
import dimdoors.common.dungeon.pack.DungeonType;
import dimdoors.common.items.ItemDimensionalDoor;
import dimdoors.common.util.FileFilters;
...
public class DungeonHelper {
	...
	private List<DungeonData> untaggedDungeons = Lists.newArrayList();
	private List<DungeonData> registeredDungeons = Lists.newArrayList();

	private DungeonPack RuinsPack;
	private DungeonPack NetherPack;
	private Map<String, DungeonPack> dungeonPackMapping = Maps.newHashMap();
	private List<DungeonPack> dungeonPackList = Lists.newArrayList();
	...
	private static List<String> parseDungeonNames(List<DungeonData> dungeons) {
		String name;
		File schematic;
	 ...
	 */
	public static List<DungeonData> getDungeonChainHistory(NewDimData start, DungeonPack pack, int maxSize) {
		if (start == null) {
		...
		List<DungeonData> history = Lists.newArrayList();
	 ...
	 */
	public static List<DungeonData> listDungeonsInTree(NewDimData root, DungeonPack pack, int maxSize) {
		int count = 0;
		NewDimData current;
		DungeonData dungeon;
		List<DungeonData> dungeons = Lists.newArrayList();
		Queue<NewDimData> pendingDimensions = Lists.newLinkedList();
	...
	public static NewDimData getAncestor(NewDimData dimension, DungeonPack pack, int maxLevels) {
		...
		for (int levels = 0; levels <= maxLevels; levels++) {
			if (parent == null || parent.dungeon() == null || parent.dungeon().dungeonType().Owner != pack) {
				break;
			}
			current = parent;
	...
	private void registerDungeons() {
		File file = new File(DimDoorsConfig.customSchematicDirectory);
		if (file.exists() || file.mkdir()) {
			FileHelper.copyFile(DUNGEON_CREATION_GUIDE_SOURCE_PATH, file.getAbsolutePath() + "/How_to_add_dungeons.txt");
		}

		DungeonPackConfigReader reader = new DungeonPackConfigReader();
		registerBundledDungeons(reader);
		registerCustomDungeons(DimDoorsConfig.customSchematicDirectory, reader);
	}
	...
	public DimLink createCustomDungeonDoor(World world, int x, int y, int z) {
		...
		ItemDimensionalDoor.placeDoor(world, new BlockPos(x, y, z), EnumFacing.VALUES[3], DimBlocks.warpDoor, false);
		...
		if (packDirectories != null) {
			...
			for (File packDirectory : packDirectories) {
				...
				if (packFiles != null) {
					...
					packFilePaths = Lists.newArrayList();
	...
	public boolean exportDungeon(World world, int centerX, int centerY, int centerZ, String exportPath) {
		...
		try {
			DungeonSchematic dungeon = DungeonSchematic.copyFromWorld(world, centerX - MAX_EXPORT_RADIUS, centerY - MAX_EXPORT_RADIUS, centerZ - MAX_EXPORT_RADIUS, MAX_DUNGEON_WIDTH, MAX_DUNGEON_HEIGHT, MAX_DUNGEON_LENGTH, true);
			dungeon.applyExportFilters();
	...
	private DungeonPack getRandomDungeonPack(DungeonPack current, Random random) {
		...
		List<WeightedContainer<DungeonPack>> packs = Lists.newArrayList();
	...
	public List<String> getDungeonNames() {
		...
		HashSet<String> dungeonNames = new HashSet<>();
