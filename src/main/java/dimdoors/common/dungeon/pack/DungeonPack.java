...
public class DungeonPack
{
	...
	public DungeonPack(DungeonPackConfig config)
	{
		...
		int index;
		int maxLength = 0;
		this.allDungeons = Lists.newArrayList();
		this.nameToTypeMapping = Maps.newHashMap();
		this.groupedDungeons = Lists.newArrayList();
		...
		for (String typeName : config.getTypeNames())
		{
			...
			this.groupedDungeons.add(Lists.newArrayList());
			index++;
		}
		
		//Construct optimized rules from definitions
		List<DungeonChainRuleDefinition> definitions = config.getRules();
	...
	public DungeonData getNextDungeon(NewDimData parent, Random random)
	{
		...
		List<DungeonData> history = DungeonHelper.getDungeonChainHistory(parent, this, maxSearchLength);

		List<DungeonData> subtreeHistory = null;
		...
		{
			subtreeHistory = Lists.newArrayList();
		}

		return getNextDungeon(history, subtreeHistory, random);
	}
	
	private DungeonData getNextDungeon(List<DungeonData> history, List<DungeonData> subtreeHistory, Random random)
	{
		...
		for (DungeonChainRule rule : rules)
		{
			if (rule.evaluate(typeHistory))
			{
				...
				List<WeightedContainer<DungeonType>> products = rule.products();
				...
				do
				{
					...
					if (nextType != null)
					{
						...
						if (excludedDungeons == null && doExclude)
						{
							...
							{
								excludedDungeons = Sets.newHashSet();
	...
	private static DungeonType getRandomDungeonType(Random random, List<WeightedContainer<DungeonType>> types,
	...
	private static DungeonData getRandomDungeon(Random random, Collection<DungeonData> dungeons)
	{
		...
		List<WeightedContainer<DungeonData>> weights = Lists.newArrayList();
		...
		return resultContainer.getData();
	}
	@Override
