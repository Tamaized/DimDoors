...
import dimdoors.common.util.BaseConfigurationProcessor;
import dimdoors.common.util.ConfigurationProcessingException;
...
public class DungeonPackConfigReader extends BaseConfigurationProcessor<DungeonPackConfig> {
	...
	public DungeonPackConfig readFromStream(InputStream inputStream) throws ConfigurationProcessingException {
		...
		try {
			...
			config.setTypeNames(Lists.newArrayList());
			config.setRules(Lists.newArrayList());
	...
	private interface ILineProcessor {
		void process(String line, DungeonPackConfig config) throws ConfigurationProcessingException;
	}
